...
function searchTableCreate() {
    ...
    $("#submit").click(function () {
        ...
        if (sendKey) {
            var url = hostname + "search?field=" + sendField + "&id=" + sendKey;
...
function addData() {
    ...
    var url = hostname + "add?date=" + extractedDate + "&time=" + extractedTime + "&state=" + extractedState + "&city=" + extractedCity + "&address=" + extractedAddress;
    var tempArr = [];
...
function deleteData(row) {
    if (editing == false) {
        ...
        var url = hostname + "delete?date=" + tempData[0] + "&time=" + tempData[1] + "&state=" + tempData[2] + "&city=" + tempData[3] + "&address=" + tempData[4];
        $.get(url, function (data) {
...
function editData(row) { //get which row, then after row is changed get what changed and send to server
    ...
    if (row.innerHTML == "Edit") {
        if (editing == false) {
            ...
            var url = hostname + "edit?old=" + previousData;
            $.get(url, function (data) {
                var parent = document.getElementById('table');
                if (data == false) {
        ...
        getUniqueValues();
        var url = hostname + "edit?old=" + previousData + "&new=" + updatedData;
        $.get(url, function (data) {
            var parent = document.getElementById('table');
            if (data == false) {
